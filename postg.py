...
class DBexecutor:
    ...
    def create_db(self):
        if self.curs:
            self.curs.execute(
                """CREATE TABLE student (id serial PRIMARY KEY, name varchar(100), gpa numeric(10, 2), birth timestamp with time zone);""")
            self.curs.execute("""CREATE TABLE course (id serial PRIMARY KEY, name varchar(100));""")
            self.curs.execute(
                """CREATE TABLE student_course (id serial PRIMARY KEY, student_id INTEGER REFERENCES student(id), course_id INTEGER REFERENCES  course(id));""")
    ...
    def add_student(self, student):
        if self.curs:
            self.curs.execute("INSERT INTO student (name, gpa, birth) VALUES (%s, %s, %s)",
                              (student['name'], student['gpa'], student['birth']))

    def add_references(self, student_id, course_id):
        if self.curs:
            self.curs.execute("INSERT INTO student_course (student_id, course_id) VALUES (%s, %s)",
    ...
    def get_students(self, course_id):
        if self.curs:
            self.curs.execute(
                """SELECT s.id, s.name, c.name FROM student_course sc JOIN student s ON s.id = sc.student_id JOIN 
                course c ON c.id = sc.course_id WHERE course_id=%s""",
                (course_id,))
